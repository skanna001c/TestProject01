...
public class NewConnectTest extends ComcastTest {
	...
	/*
	 * @BeforeMethod public void beforeMethod() { //check for rerun and the
	 * status of the method if (userName==null) {
	 * userName="ordermanagementbat1"; } else
	 * userName=getDataDump().getValue("userName");
	 * 
	 * centuryApplication = new CenturyApplication(browser, report); // CM nd
	 * CSO login -> added by rijin on 8/18/2056 if
	 * (getDataDump().getValue("CM_Status").equalsIgnoreCase("PASS") &&
	 * !(getDataDump().getValue("CSOLoggedIN").equalsIgnoreCase("PASS"))) {
	 * centuryApplication.openCSOUrl(userName);
	 * getDataDump().setValue("CSOLoggedIN","PASS"); } else if
	...
	public void selectServiceEPLCoax() throws InterruptedException {
		...
		if ((new ServiceTabPageCM(browser, report)).SelectPricePlan()) {
			if ((new ServiceTabPageCM(browser, report)).EPL()) {
				...
				Assert.fail(" Select EPL plan failed");
		} else
	...
	public void selectServiceEVPL() throws InterruptedException, AWTException {
		(new ServiceTabPageCM(browser, report)).ClickOnServiceTab();
		if ((new ServiceTabPageCM(browser, report)).SelectPricePlan()) {
			if ((new ServiceTabPageCM(browser, report)).EVPL()) {
				if ((new ServiceTabPageCM(browser, report)).EquipmentFee(serviceInfo)) {
					if ((new ServiceTabPageCM(browser, report)).ClickOnContinueButton()) {
				...
				Assert.fail(" Select EVPL plan failed");
		} else
			Assert.fail(" Select service plan failed");
	...
	@Test(priority = 515)
	@PerfTransaction(name = "SelectServiceENS")
	public void selectServiceENS() throws InterruptedException, AWTException {
		(new ServiceTabPageCM(browser, report)).ClickOnServiceTab();
		if ((new ServiceTabPageCM(browser, report)).SelectPricePlan()) {
			if ((new ServiceTabPageCM(browser, report)).ENS()) {
				if ((new ServiceTabPageCM(browser, report)).EquipmentFee(serviceInfo)) {
					if ((new ServiceTabPageCM(browser, report)).ClickOnContinueButton()) {
				...
				Assert.fail(" Select ENS plan failed");
		} else
			Assert.fail(" Select service plan failed");
		...
		if ((new ProcessTabPageCM(browser, report)).UNIConfiguration(processInfo, Site1)) {
			...
			if ((new ProcessTabPageCM(browser, report)).EVCConfiguration_EDI(processInfo)) {
				if ((new ProcessTabPageCM(browser, report)).EqFeeConfiguration(processInfo)) {
					...
					Assert.fail("Equipment fee configuration failed");
			} else
	...
	public void processServiceEPLCoax() throws InterruptedException {
		...
		if ((new ProcessTabPageCM(browser, report)).UNIConfiguration(processInfo, Site1)) {
			...
			if ((new ProcessTabPageCM(browser, report)).UNI2Configuration(processInfo, Site2)) {
				...
				Assert.fail("UNI2 configuration failed");
		} else
			Assert.fail("UNI configuration failed");
	}
	...
	@Test(priority = 620)
	@PerfTransaction(name = "ProcessServiceEVPL")
	public void processServiceEVPL() throws InterruptedException {
		if (getDataDump().getValue("processService_status").equalsIgnoreCase("FAIL")) {
			selectService();
		}
		SRID = (new ProcessTabPageCM(browser, report)).ProcessConfiguration(processInfo);
		...
		if ((new ProcessTabPageCM(browser, report)).UNIConfiguration(processInfo, Site1)) {
			if ((new ProcessTabPageCM(browser, report)).UNI2Configuration(processInfo, Site2)) {
				if((new ProcessTabPageCM(browser, report)).UNI3Configuration(processInfo, Site3)){
				if ((new ProcessTabPageCM(browser, report)).EVCConfiguration_EVPL(processInfo)) {
					if((new ProcessTabPageCM(browser, report)).EVC2Configuration_EVPL(processInfo)){
					if ((new ProcessTabPageCM(browser, report)).EqFeeConfiguration(processInfo)) {
						if ((new ProcessTabPageCM(browser, report)).EqFee2Configuration(processInfo)){
							if ((new ProcessTabPageCM(browser, report)).EqFee3Configuration(processInfo)){
						if ((new ProcessTabPageCM(browser, report)).ClickOnContinueButton()) {
						  } else Assert.fail("Click on continue button failed");
							}else Assert.fail("Equipment fee3 configuration failed");
						} else Assert.fail("Equipment fee2 configuration failed");
					} else
						Assert.fail("Equipment fee configuration failed");
					}else Assert.fail("EVC2 configuration failed");
				} else
					Assert.fail("EVC configuration failed");
				}Assert.fail("UNI3 configuration failed");
			} else
				Assert.fail("UNI2 configuration failed");
		} else
			Assert.fail("UNI configuration failed");
	...
	@Test(priority = 630)
	@PerfTransaction(name = "ProcessServiceENS")
	public void processServiceENS() throws InterruptedException {
		if (getDataDump().getValue("processService_status").equalsIgnoreCase("FAIL")) {
			selectService();
		}
		SRID = (new ProcessTabPageCM(browser, report)).ProcessConfiguration(processInfo);
		...
		if ((new ProcessTabPageCM(browser, report)).UNIConfiguration(processInfo, Site1)) {
			if ((new ProcessTabPageCM(browser, report)).UNI2Configuration(processInfo, Site2)) {
				if ((new ProcessTabPageCM(browser, report)).EVCConfiguration_ENS(processInfo)) {
					if((new ProcessTabPageCM(browser, report)).EVC2Configuration_ENS(processInfo)){
					if ((new ProcessTabPageCM(browser, report)).EqFeeConfiguration(processInfo)) {
						if ((new ProcessTabPageCM(browser, report)).EqFee2Configuration(processInfo)){
						if ((new ProcessTabPageCM(browser, report)).ClickOnContinueButton()) {
						  } else Assert.fail("Click on continue button failed");
	...
	public void submitOrder() throws InterruptedException, AWTException {
		...
		// if((new OrderSummaryTabCMPage(browser,
		// report)).assignLabel(orderSummaryInfo)){
		if ((new OrderSummaryTabCMPage(browser, report)).enterOrderDetails(orderSummaryInfo)) {
			if ((new OrderSummaryTabCMPage(browser, report)).mrcNrc_Value(orderSummaryInfo)) {
				if ((new OrderSummaryTabCMPage(browser, report)).ClickSubmitOrderButton()) {
					...
					Assert.fail("Order submission failed");
			} else
				Assert.fail("Entering NRC values failed");
		} else
			Assert.fail("Entering order detrails failed");
		...
		if ((new OrderSummaryTabCMPage(browser, report)).enterOrderDetails(orderSummaryInfo)) {
			if ((new OrderSummaryTabCMPage(browser, report)).mrcNrc_Value(orderSummaryInfo)) {
				if ((new OrderSummaryTabCMPage(browser, report)).Attachments(orderSummaryInfo)) {
					...
					Assert.fail("Uploading the Tax Excemption form failed");
			} else
				Assert.fail("Entering NRC values failed");
		} else
	 ...
	 * report)).NavigateToCSO(orderSummaryInfo); (new
	 * WorkOrderTabPageCSO(browser,
	 * report)).SearchForOrderInSO(getDataDump().getValue("SRID_RT")); (new
	 * WorkOrderTabPageCSO(browser,
	 * report)).ClickFirstFiberSiteFlow(getDataDump().getValue("FiberSite1_RT"))
	 * ;
	 * 
	 * }
	 */
	...
	/*
	 * @Test(priority=810) public void SearchOrderndClickFirstCoaxSiteFlow() {
	 * //(new OrderSummaryTabCMPage(browser,
	 * report)).NavigateToCSO(orderSummaryInfo); (new
	 * WorkOrderTabPageCSO(browser,
	 * report)).SearchForOrderInSO(getDataDump().getValue("SRID_RT")); (new
	 * WorkOrderTabPageCSO(browser,
	 * report)).ClickFirstCoaxSiteFlow(getDataDump().getValue("CoaxSite1_RT"));
	 * 
	 * }
	 * 
	 * @Test(priority=815) public void SearchOrderndClickSecondCoaxSiteFlow() {
	 * //(new OrderSummaryTabCMPage(browser,
	 * report)).NavigateToCSO(orderSummaryInfo); (new
	 * WorkOrderTabPageCSO(browser,
	 * report)).SearchForOrderInSO(getDataDump().getValue("SRID_RT")); (new
	 * WorkOrderTabPageCSO(browser,
	 * report)).ClickFirstCoaxSiteFlow(getDataDump().getValue("CoaxSite2_RT"));
	 * 
	 * }
	 */
	...
	public void Conduct_Coax_Survey() throws InterruptedException, AWTException {
		/*
		 * if (getDataDump().getValue("Conduct_Coax_Survey_status").
	...
	public void Obtain_Coax_Permit() throws InterruptedException, AWTException {
		/*
		 * if
		 * (getDataDump().getValue("Obtain_Coax_Permit_status").equalsIgnoreCase
		 * ("fail")) {
		 */
		SearchOrderndLaunchCoaxSiteFlow();
		// }
	...
	public void Complete_Site_Build() throws InterruptedException, AWTException {
		...
		// if
		// (getDataDump().getValue("Complete_Site_Build_status").equalsIgnoreCase("fail"))
		// {
		SearchOrderndLaunchFiberSiteFlow();
		// }
	...
	public void Complete_Site_Build_Coax() throws InterruptedException, AWTException {
		// if
		// (getDataDump().getValue("Complete_Site_Build_Coax_status").equalsIgnoreCase("fail"))
		// {
		SearchOrderndLaunchCoaxSiteFlow();
		// }
	...
	public void Obtain_Fiber_Plant_Permit() throws InterruptedException, AWTException {
		// if
		// (getDataDump().getValue("Obtain_Fiber_Plant_Permit_status").equalsIgnoreCase("fail"))
		// {
		SearchOrderndLaunchFiberSiteFlow();
		// }
	...
	public void Complete_Fiber_Plant_Build() throws InterruptedException, AWTException {
		// if
		// (getDataDump().getValue("Complete_Fiber_Plant_Build_status").equalsIgnoreCase("fail"))
		// {
		SearchOrderndLaunchFiberSiteFlow();
		// }
	...
	public void Complete_Coax_Build() throws InterruptedException, AWTException {
		// if
		// (getDataDump().getValue("Complete_Coax_Build_status").equalsIgnoreCase("fail"))
		// {
		SearchOrderndLaunchCoaxSiteFlow();
		// }
	...
	public void Assign_Design_Info() throws InterruptedException {
		...
		// report)).NavigateToCSO(orderSummaryInfo);
		// if
		// (getDataDump().getValue("Assign_Design_Info_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
	...
	public void Generate_Core_Config() throws InterruptedException {
		// if
		// (getDataDump().getValue("Generate_Core_Config_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
		// (new OrderSummaryTabCMPage(browser,
	...
	public void Generate_CPE_Config() throws InterruptedException {
		// if
		// (getDataDump().getValue("Generate_CPE_Config_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
		// (new OrderSummaryTabCMPage(browser,
	...
	public void Install_CPE_Coax() throws InterruptedException, AWTException {
		// if
		// (getDataDump().getValue("Install_CPE_Coax_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
		(new ServiceLevelTasks(browser, report)).InstallCPE();
	...
	public void Activate_Service() throws InterruptedException {
		// if
		// (getDataDump().getValue("Activate_Service_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
	...
	public void Notify_Customer_of_Service_Installation() throws InterruptedException {
		// if
		// (getDataDump().getValue("Notify_Customer_of_Service_Installation_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
	...
	public void Complete_Customer_Acceptance_Testing(Method method) throws InterruptedException {
		...
		// if
		// (getDataDump().getValue("Complete_Customer_Acceptance_Testing_status").equalsIgnoreCase("fail")
	...
	public void Create_Order_Billing_Package() throws AWTException, InterruptedException {
		// if
		// (getDataDump().getValue("Create_Order_Billing_Package_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
	...
	public void EquipmentFeeFlow() throws InterruptedException {
		// if
		// (getDataDump().getValue("EquipmentFeeFlow_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
		// (new OrderSummaryTabCMPage(browser,
	...
	public void EqFeeStartBilling() throws InterruptedException {
		// if
		// (getDataDump().getValue("EqFeeStartBilling_status").equalsIgnoreCase("fail"))
		// {
		EDIFlow();
		// }
